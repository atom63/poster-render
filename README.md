...
Node.js CLI for rendering Xiaohongshu (XHS) carousel poster slides as PNG images. It accepts structured JSON and outputs 1080×1350px slides with typography, syntax highlighting, background patterns, and cover image support.
...
Without font files, system fallbacks are used. For Western text, the defaults are Helvetica/Georgia/Menlo; for Chinese/CJK text, install a CJK-capable font on the machine (for example PingFang SC, Hiragino Sans GB, Microsoft YaHei, or Noto Sans CJK SC) to avoid tofu and spacing issues.
...
Sections are **auto-paginated**: if a section body is too tall for a slide, it is split across multiple slides automatically. Sections with images are treated as page-break boundaries, so they usually start a new slide when they would otherwise follow earlier content.
...
Tables render as real table cards, and tall tables paginate across slides with the header repeated on continuation pages. Code blocks wrap long tokens and expressions safely at the character level, so long identifiers stay inside the code card instead of overflowing.
...
The cover kicker is generated by default from the deck text as `全文 {字数}字 · {阅读时间}分钟阅读`. Turn it off with `cover.showKicker: false` in JSON or `--no-cover-kicker` on the CLI.
...
This repository is already solid for local use and open-source development: it includes package metadata, installable bin entries, a license, a sample input file, basic verification scripts, and node:test coverage for table pagination and wrapped code rendering. A production-ready release would mainly need CI/release automation and a published npm version.

## Contributing

Small patches are preferred. Run `npm test` and `npm run lint` before sending a PR, and keep `README.md` aligned with the actual CLI behavior and sample assets.
