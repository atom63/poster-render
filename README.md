...
The top-level `template` field selects the layout family and defaults to `default`. `boardStyle` is accepted as a backward-compatible alias for now.

The cover kicker is auto-generated by default from the deck text as `全文 {字数}字 · {阅读时间}分钟阅读`. Turn it off with `cover.showKicker: false` in JSON or `--no-cover-kicker` on the CLI.
